...
import { VoiceSelectionPopup } from './voice/VoiceSelectionPopup';
...
export const AIChat = ({ isOpen, onClose, property }: AIChatProps) => {
  ...
  // Show voice popup when chat opens and no voice is selected
  useEffect(() => {
    ...
    <>
      <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-sm">
        <div className="absolute right-0 top-0 h-full w-full bg-white">
                  ...
                  {!isListening && !isSpeaking && (
                    <div className="flex items-center space-x-2">
                      <p className="text-sm text-gray-500">
                        {selectedVoiceName ? `${selectedVoiceName} (${voiceMode === 'ultravox' ? 'Ultravox' : 'Browser'})` : 'AI Assistant'}
                      </p>
                      {voiceMode === 'ultravox' && (
          ...
          {/* Voice Controls - Only show if voice is selected */}
          ...
          {/* Show message when no voice is selected */}
          {!selectedVoiceId && (
            <div className="p-4 border-b border-gray-200">
              <div className="text-center">
                <p className="text-gray-500 mb-4">Please select a voice to start the conversation</p>
                <Button
